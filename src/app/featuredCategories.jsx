    ...
    return (
        <div className="grid grid-cols-3 gap-4 p-2 mx-2 mt-4 sm:grid-cols-4 lg:grid-cols-5">
            {categories.map((cat,index) => (
                <div className="border-2 border-pink-400 rounded-md flex p-1 flex-col shadow-pink-lg hover:shadow-[0_0_40px_pink] transition-shadow duration-150" key={index}>
                    <Image src={cat.image} alt={cat.name} />
                    <h3 className="text-center font-semibold text-pink-600 mb-3">{cat.name}</h3>
                </div>
            ))}
        </div>
    );
